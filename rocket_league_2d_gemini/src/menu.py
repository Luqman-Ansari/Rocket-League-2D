...
class Button:
    ...
    def draw(self, screen):
        ...
        shadow_rect = shadow_surf.get_rect(
            center=(self.rect.centerx + 3, self.rect.centery + 3)
        )
...
class MenuManager:
    ...
    def __init__(self, screen, clock):
        ...
        self.btn_back = Button(
            "BACK", center_x - 100, HEIGHT - 100, 200, 60, action="BACK"
        )
        self.btn_play = Button(
            "P L A Y", center_x - 100, HEIGHT - 200, 200, 60, action="PLAY"
        )
    ...
    def _draw_header(self):
        ...
        if self.state == "MAIN" and logo:
            ...
            if self.state == "AUTOSELECT":
                title_text = (
                    "SELECT BOT 1 (LEFT/BLUE)"
                    if self.auto_selection_stage == 1
                    else "SELECT BOT 2 (RIGHT/RED)"
                )
            else:
    ...
    def _draw_duration_input(self):
        ...
        lbl = assets_loader.FONTS["ui"].render(
            "ENTER MATCH DURATION (Seconds):", True, BLUE
        )
        ...
        txt_surf = assets_loader.FONTS["title"].render(
            self.duration_input_text, True, WHITE
        )
        self.screen.blit(
            txt_surf,
            (
                input_rect.centerx - txt_surf.get_width() // 2,
                input_rect.centery - txt_surf.get_height() // 2,
            ),
        )

        inst = assets_loader.FONTS["body"].render(
            "Press ENTER or click PLAY to Start Match", True, ORANGE
        )
        ...
        for key, base_rect in self.mode_rects.items():
            ...
            if field_tex:
                thumb = pygame.transform.scale(
                    field_tex, (draw_rect.width, draw_rect.height)
                )
            ...
            # Border
            border_color = (
                ORANGE if is_selected else (WHITE if is_hover else LIGHT_GRAY)
            )
            ...
            self.screen.blit(
                name_surf,
                (draw_rect.centerx - name_surf.get_width() // 2, draw_rect.top + 120),
            )
            ...
            self.screen.blit(
                d1, (draw_rect.centerx - d1.get_width() // 2, draw_rect.top + 180)
            )
            self.screen.blit(
                d2, (draw_rect.centerx - d2.get_width() // 2, draw_rect.top + 210)
            )
    ...
    def _draw_playmode_selection(self):
        ...
        info = assets_loader.FONTS["body"].render(
            "Choose your play mode", True, LIGHT_GRAY
        )
    ...
    def _draw_bot_selection(self):
        ...
        hint = assets_loader.FONTS["body"].render(
            f"Selected: {self.opponent_type}", True, ORANGE
        )
        ...
        if self.auto_selection_stage == 1:
            ...
            if self.bot1_type:
                selected = assets_loader.FONTS["body"].render(
                    f"Bot 1: {self.bot1_type}", True, BLUE
                )
                self.screen.blit(
                    selected, (WIDTH // 2 - selected.get_width() // 2, HEIGHT - 160)
                )
        else:
            info = assets_loader.FONTS["body"].render(
            ...
            self.screen.blit(
                bot1_txt, (WIDTH // 2 - bot1_txt.get_width() // 2, HEIGHT - 180)
            )
            if self.bot2_type:
                bot2_txt = assets_loader.FONTS["body"].render(
                    f"Bot 2 (Red): {self.bot2_type}", True, RED
                )
                self.screen.blit(
                    bot2_txt, (WIDTH // 2 - bot2_txt.get_width() // 2, HEIGHT - 140)
                )
    ...
    def _draw_auto_need_message(self):
        ...
        y = 200
        msg1 = assets_loader.FONTS["ui"].render(
            "AUTO mode requires at least 2 AI bots", True, ORANGE
        )
        msg2 = assets_loader.FONTS["body"].render(
            "Currently available: 1 bot (Basic)", True, LIGHT_GRAY
        )
        msg3 = assets_loader.FONTS["body"].render(
            "Train more models and save them to rl/versions/", True, LIGHT_GRAY
        )
        msg4 = assets_loader.FONTS["body"].render(
            "to unlock AUTO mode", True, LIGHT_GRAY
        )
    ...
    def _handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            ):  # Only propagate meaningful results
                return result
    ...
    def _scan_bot_models(self):
        ...
        buttons.append(
            Button("Basic Bot", center_x - 150, y_offset, 300, 50, action="BOT:Basic")
        )
        y_offset += 60
        ...
        print(f"[DEBUG] Scanning for bot models in: {versions_path}")
        print(f"[DEBUG] Directory exists: {os.path.exists(versions_path)}")
        ...
        if os.path.exists(versions_path) and os.path.isdir(versions_path):
            ...
            print(f"[DEBUG] Found {len(zip_files)} .zip files: {zip_files}")
            ...
            print(f"[DEBUG] Versions directory not found or not a directory")
