...
class HUD:
    ...
    def draw(screen, score, time_left, winner_text=""):
        """Draw the game HUD."""
        # Score
        score_txt = assets_loader.FONTS["header"].render(
            f"{score[0]} - {score[1]}", True, WHITE
        )
    ...
    def draw_pause_menu(screen):
        ...
        m = assets_loader.FONTS["body"].render(
            "[M] Menu   [R] Restart   [Q] Quit", True, ORANGE
        )
...
class Match:
    ...
    def _init_players(self):
        ...
        if self.opponent_type == "HUMAN":
            ...
            if bot1_type == "Basic":
                self.p1 = SimpleAICar(
                    200, HEIGHT // 2, BLUE, "car_blue", self.friction_car
                )
            else:
                self.p1 = TrainedAICar(
                    200, HEIGHT // 2, BLUE, "car_blue", self.friction_car
                )
            ...
            if bot2_type == "Basic":
                self.p2 = SimpleAICar(
                    WIDTH - 200, HEIGHT // 2, RED, "car_red", self.friction_car
                )
            else:
                self.p2 = TrainedAICar(
                    WIDTH - 200, HEIGHT // 2, RED, "car_red", self.friction_car
                )
        ...
        self.gk1 = Goalkeeper(
            50, HEIGHT // 2, DARK_BLUE, "left", "gk_blue", self.friction_car
        )
        self.gk2 = Goalkeeper(
            WIDTH - 50, HEIGHT // 2, DARK_RED, "right", "gk_red", self.friction_car
        )
    ...
    def _load_ai_model(self, model_name, player="p1"):
        ...
        print(f"[DEBUG] Attempting to load {player} bot: {model_name}")
    ...
    def _check_goals(self):
        ...
        if (
            ...
            and GOAL_TOP_Y < self.ball.y < GOAL_BOTTOM_Y
        ):
            self.score[1] += 1
            self.goal_timer = 90
    ...
    def _draw_field(self):
        ...
        if field_img:
            ...
            pygame.draw.line(
                self.screen, WHITE, (WIDTH // 2, 0), (WIDTH // 2, HEIGHT), 3
            )
    ...
    def _draw_goals(self):
        ...
        pygame.draw.rect(
            self.screen, WHITE, (WIDTH - 60, GOAL_TOP_Y, 60, GOAL_WIDTH), 3
        )
...
def run_match(screen, clock, mode_config):
        ...
        if opponent_type == "Basic":
            ...
            if SB3_AVAILABLE:
                model_path = os.path.join(
                    "..", "rl", "versions", f"{opponent_type}.zip"
                )
                if os.path.exists(model_path):
                    try:
                        ...
                        # Fallback to basic AI
                        p1 = SimpleAICar(
                            200, HEIGHT // 2, BLUE, "car_blue", friction_car
                        )
                else:
                    print(f"Model file not found: {model_path}")
                    # Fallback to basic AI
    ...
    while True:
        ...
        if game_state == "PLAYING":
            ...
            if goal_timer == 0:
                ...
                if ball.x - ball.radius < 0 and GOAL_TOP_Y < ball.y < GOAL_BOTTOM_Y:
                    ...
                    ball.x + ball.radius > WIDTH and GOAL_TOP_Y < ball.y < GOAL_BOTTOM_Y
                ):
                    score[0] += 1
                    goal_timer = 90
                    if assets_loader.SOUNDS["goal"]:
                ...
                if goal_timer == 0:
                    ball.reset()
                    p1.x, p1.y = 200, HEIGHT // 2
                    p1.vx = p1.vy = 0
                    p2.x, p2.y = WIDTH - 200, HEIGHT // 2
                    p2.vx = p2.vy = 0
                    gk1.x, gk1.y = 50, HEIGHT // 2
                    gk1.vx = gk1.vy = 0
                    gk2.x, gk2.y = WIDTH - 50, HEIGHT // 2
                    gk2.vx = gk2.vy = 0
        ...
        # HUD / Overlays
        draw_hud(
            screen, score, time_left, winner_text if game_state == "GAMEOVER" else ""
        )
